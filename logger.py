...
def log_info(args, logger):
    ...
    for k, v in args.data.__dict__.items():
        logger.info(f"{k}: {v}")

    for k, v in args.encoder_params.__dict__.items():
        logger.info(f"{k}: {v}")
    for k, v in args.decoder_params.__dict__.items():
        logger.info(f"{k}: {v}")
    for k, v in args.diffusion_params.__dict__.items():
        logger.info(f"{k}: {v}")
    for k, v in args.loss_weights.__dict__.items():
        logger.info(f"{k}: {v}")
    for k, v in args.training.__dict__.items():
        logger.info(f"{k}: {v}")    
